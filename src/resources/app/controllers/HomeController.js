...
class HomeController {
    ...
    async marketplacever1(req, res, next) {
        ...
        client.end;
        res.render('market/marketplacever1', {
            layout: null,
            data_postgre_string: data_postgre_string
        });
    }

    // take account 
    async take_account(req, res, next) {
		console.log("take_account", req);
        if (req.body) {
            account_login = req.body.acc;
			console.log("Authenticated with " + account_login)
            res.json("Xác thực thành công!")
        } else {
            res.json("Không có body truyền lên. Không get được account!")
        }
    }
    ...
    async myorder(req, res, next) {
		...
		console.log("data_order_array: ", data_order_array)
        res.render('market/myorder', {
            layout: null,
            data_order_array: data_order_array
        });
    }
